...
int main(int argc, char **argv)
{
	...
	if (argc != 4) {
		fprintf(stderr,
			"Usage: %s <original_pe> <binary_file> <new_pe>\n",
			argv[0]);
		return -1;
	}
	...
	base_start_addr =
		mmap(NULL, sb.st_size, PROT_READ, MAP_SHARED, base_fd, 0);
	...
	int32_t pe_hdr_offset = get_pehdr_offset((const char *)base_start_addr);
	if (pe_hdr_offset < 0) {
		fprintf(stderr, "Fail to get PE header\n");
		return -1;
	}
	...
	uint32_t section_table_offset =
		pe_hdr_offset + sizeof(struct pe_hdr) + pe_hdr->opt_hdr_size;
	base_sections = (struct section_header *)(base_start_addr +
	...
	for (i = 0; i < num_sections; i++) {
		...
		if (zheader->payload_offset >= sect->data_addr &&
		    zheader->payload_offset <
			    (sect->data_addr + sect->raw_data_size)) {
			payload_sect_idx = i;
			payload_sect_off =
				zheader->payload_offset - sect->data_addr;
			section_found = true;
		}
	}
	if (!section_found) {
		fprintf(stderr,
			"Payload offset not found within any section\n");
		return -1;
	}

	/* Calculate the end of the last section in virtual memory */
	for (i = 0; i < num_sections; i++) {
		uint32_t section_end = base_sections[i].virtual_address +
				       base_sections[i].virtual_size;
	...
	struct section_header *new_sections =
		calloc(1, new_hdr->sections * sizeof(struct section_header));
	if (!new_sections) {
		perror("Failed to allocate memory for new section headers");
		return -1;
	}
	memcpy(new_sections, base_sections,
	       pe_hdr->sections * sizeof(struct section_header));

	/* Configure the new .bpf section */
	struct section_header *bpf_section =
		&new_sections[new_hdr->sections - 1];
	...
	uint32_t new_size_of_image =
		bpf_section->virtual_address + bpf_section->virtual_size;
	...
	size_t section_table_size =
		new_hdr->sections * (sizeof(struct section_header));
	...
	if (base_sections[0].data_addr < aligned_headers_size) {
		for (i = 0; i < new_hdr->sections; i++) {
			...
			current_offset += ALIGN_UP(
				new_sections[i].raw_data_size, file_align);
		}
	...
	payload_new_offset =
		new_sections[payload_sect_idx].data_addr + payload_sect_off;
	/* Update */
	...
	msync(out_start_addr,
	      new_sections[i].data_addr + new_sections[i].raw_data_size,
	      MS_ASYNC);
	/* For the bpf section */
	out_cur = out_start_addr + new_sections[i].data_addr;
