...
int BPF_PROG(parse_pe, struct kexec_context *context, unsigned long parser_id)
{
	...
	if (!buf) {
		bpf_printk(
			"Err: fail to reserve ringbuf to parse zboot header\n");
		return 0;
	}
	...
	if (payload_offset > image_sz || payload_size > image_sz ||
	    payload_offset > image_sz - payload_size) {
		bpf_ringbuf_discard(buf, BPF_RB_NO_WAKEUP);
	...
	ret = bpf_buffer_parser(buf, sizeof(struct cmd_hdr) + payload_size - 4,
				bpf);
	if (ret < 0) {
		bpf_ringbuf_discard(buf, BPF_RB_NO_WAKEUP);
