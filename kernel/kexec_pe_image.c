...
{
	Elf_Shdr *shdr, *symtab_shdr, *strtab_shdr, *shstrtab_shdr, *dst_shdr;
	...
	for (i = 0; i < ehdr->e_shnum; i++) {
		if (shdr[i].sh_type == SHT_SYMTAB)
			symtab_shdr = &shdr[i];
		/* GCC has .strtab and .shstrtab separated */
		else if (shdr[i].sh_type == SHT_STRTAB && i != ehdr->e_shstrndx)
			strtab_shdr = &shdr[i];
	}
	/*
	 * Clang combines .strtab and .shstrtab into one, so the strtab section
	 * index can be ehdr->e_shstrndx
	 */
	if (!strtab_shdr && ehdr->e_shstrndx < ehdr->e_shnum)
